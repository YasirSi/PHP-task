<?php
    ...
    if($result->num_rows>0)
    {
        while($row=mysqli_fetch_array($result))
        {
            ...
            {
                $_SESSION['x'] = array('userid'=>$row['user_id'],'username'=>$row['user_name'], 'role'=>$row['role'],  'password'=>$row['password'], 'email'=>$row['user_email']);
            }
        }
    }
    ...
    if(isset($_POST['edituser']))
    {
        ...
        if($result->num_rows > 0)
		{
			...
			{
                $_SESSION['y']=array('username'=>$row['user_name'],'email'=>$row['user_email']);
            ...
            $x = $_SESSION['x']['userid'];
            $update = "UPDATE users set `user_name` = '$username', `password` = '$password', `user_email` = '$email' WHERE user_id = $x";
